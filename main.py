...
art_test = evaluate_theta(
    ...
    cash_start=cash_start_test,          # ← inicia donde terminó TRAIN
    cash_alloc=0.50
)

# Ahora el VALID inicia con el equity final de TEST.
cash_start_valid = float(art_test["equity"].iloc[-1])

art_valid = evaluate_theta(
    valid_x, valid_y,
    best["entry_z"], best["exit_z"], best["q"], best["r"],
    cash_start=cash_start_valid,         # ← inicia donde terminó TEST
    cash_alloc=0.50
)
